...
our $L_root_path;		## keeping an extra variable; storing path in Unix format
...
$file->add_command(-label => "Quit",-underline=>0, -command => sub {system("del *.tmp"); exit;});
...
$help->add_command(-label => "Manual",-underline=>0, -command =>\&manual);	#	\&mannual);
$help->add_command(-label => "About",-underline=>0, -command =>	\&about);#			\&about);
$help->add_command(-label => "Citation",-underline=>0, -command =>	\&citation);#			\&about);
...
#my $test_blast_prog=$frnt_top->new_ttk__progressbar(-orient => 'horizontal', -length => 100, -mode => 'determinate', -variable=>\$blast_prg1)->g_grid(-column=>0,-row=>15,-sticky=>"wn");

#my $test_blast_prog2=$frnt_top->new_ttk__progressbar(-orient => 'horizontal', -length => 100, -mode => 'determinate', -variable=>\$blast_prg2)->g_grid(-column=>0,-row=>16,-sticky=>"wn");
...
my $save_options_but=$frnt_bottom->new_button(-text=>"Save options to file",-width=>18, -state=>"disabled",-command=>sub{
 #$frnt_top->g_destroy; $frnt->configure(-padding => "0 0 0 0"); main_script(\$frnt, \$run_but); 
...
sub main_script
{
	...
	Tkx::image_create_photo( "BANER", -file => "banner.gif");
	($frm_top->new_ttk__label(-image=>'BANER'))->g_grid(-column=>0, -row=>0,-sticky=>"nwes",-columnspan=>2, -padx=>80);		
	...
	$heading->g_grid(-column=>0,-row=>0,-sticky=>"s",-padx=>180);
	

	my $p = $frm_top->new_ttk__panedwindow(-orient => 'horizontal',);
	$p->g_grid(-column=>0,-row=>1,-pady=>0,-sticky=>"nwse", );
	#$frm_top->new_ttk__progressbar(-orient => 'horizontal', -length => 100, -mode => 'determinate', -variable=>\$blast_prg3)->g_grid(-column=>0,-row=>4,-padx=>0,-pady=>1,-sticky=>"w");;
...
#	$entry_drug_cand_seq->g_grid(-column=>1,-row=>6,-padx=>0,-pady=>1,-sticky=>"w");
	...
	$p2->g_grid(-column=>0,-row=>3,-sticky=>"nwse",-pady=>2 );
	...
	my $do_ess_pro_blast=$frm_seq_app->new_button(-text=>"Perform BlastP search",-width=>25, -state=>"disabled",-command=>sub{
...
###Approach -2 PPI	#################################
	my $do_PPI_search=$frm_ppi_app->new_button(-text=>" Identify hub proteins in PPI network",-width=>30, -state=>"disabled",-command=>sub{	});
...
###################PPI END##########################	
	...
	my $save_result = $frm_top->new_button(-text=>"Export results to file",-width=>25, -state=>"disabled",-command=>sub{
...
#=cut			
	while(!(-e "$L_root_path/host_orthologs_blast1.out.txt")){
		...
		$blast_prg1=blast_progess("$root_path\\host_orthologs_blast1.out","$L_root_path/host_orthologs_blast1.out",$sequence_summary{-total_seq}-($sequence_summary{-very_short_seq}+$sequence_summary{-orthologous})); 
...
.import \"componenet.txt\" tmp2";
			...
			my (@ids_with_CC,@filtered_id,@not_filtered_id);
			...
			{	my $r=$seq_id_to_PPI_id_map{$sorted_filterd_ids[$i]};
				...
				push @ids_with_CC,$sorted_filterd_ids[$i]; 					##@ids_with_CC keeping those with Centrality measures>0; used to calc top#% hub genes
			...
			#do_broard_spectrum_ana(\$canvas,\$filter_broad_spe_but,\$save_result_but,\$search_drugBank_but,$all_t_seq);
			$save_result->configure(-state=>"normal");				#do_broard_spectrum_ana(\$canvas,\$filter_broad_spe_but,\$save_result_but,\$Save_known_targets_predicted,\$Save_novel_targets_predicted,\$Save_known_targets_predicted_tsv,$all_t_seq);
...
###do_ess_pro_blast		
	$do_ess_pro_blast->configure(-command=>sub{
		...
		if(!$Eproteome_file){
			...
			my $blast2="$blast_path -p blastp -d $Eproteome_file -i $root_path\\accepted_seq_step-3.fasta -e $e_val_2 -m $out_fmt_2 -W $word_size_2 -M $sub_matrix_2 -G $gap_open -E $gap_extns -o $root_path\\essential_protein_blast2.out -f $threshold_2 -a $use_cores"." $extra_params_BLAST2";
				...
				$blast_prg2=blast_progess("$root_path\\essential_protein_blast2.out","$L_root_path/essential_protein_blast2.out",$t); 
				sleep(3);
				Tkx::update(); 
	...
	#	$dwn_str_anal->entryconfigure("Broadspectrum analysis",-state=>"normal");
...
##args: ref_fasta_file and blast tabular out file and perc_identity_cutoff
